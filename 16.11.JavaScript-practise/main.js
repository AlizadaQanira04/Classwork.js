...
// let letter = webTechs2.filter((item)=>item===item.toLocaleUpperCase())
// console.log(letter);
...
// console.log(users.filter((item)=>item.uni==="UNEC"&&item.surname[0].toLocaleLowerCase()==="r"));

// 2.2.Show users whose age is eqaul to 22 and name start 'a'.
// console.log(users.filter((item)=>item.age==='22' && item.name[0].toLocaleLowerCase()==="a"));
// 2.3.Return new array, if user is male add in front of name 'Mr', or Ms. (e.g. name:'Mr Anar', 'Ms Aynure').
// let find = users.map((item) => {
//   if (item.gender === "male") {
...
// let trueIaLoggedIn = employees.filter((item) => item.isLoggedIn === true);

// console.log(trueIaLoggedIn);
